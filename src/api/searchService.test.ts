...
describe("SearchService", () => {
  ...
  describe("search", () => {
    ...
    it("should handle books-only results", async () => {
      const mockApiResponse = {
        data: {
          query: "books only",
          books: [
      ...
      const result = await searchService.search("books only");

      expect(result.data.books).toHaveLength(1);
      expect(result.data.urls).toHaveLength(0);
    });

    it("should handle URLs-only results", async () => {
      const mockApiResponse = {
        data: {
          query: "urls only",
          books: [],
      ...
      const result = await searchService.search("urls only");
